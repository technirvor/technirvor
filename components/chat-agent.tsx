...
export default function ChatAgent() {
  ...
  const quickActions = [
    { icon: Search, label: "Search Products", action: "Show me popular products" },
    { icon: Package, label: "Track Order", action: "I want to track my order" },
    { icon: Heart, label: "Recommendations", action: "Give me some recommendations" },
  ...
  const handleReaction = (messageId: string, reaction: 'helpful' | 'notHelpful') => {
      ...
      if (msg.id === messageId) {
        return {
          ...
          reactions: {
            ...
            notHelpful: reaction === 'notHelpful' ? !msg.reactions?.notHelpful : false
          }
        };
      }
      return msg;
    ...
    toast({
      description: reaction === 'helpful' ? "Thanks for your feedback!" : "We'll improve our responses.",
      duration: 2000,
    });
  };
  ...
  return (
    <>
      ...
      {isOpen && (
        <>
          {/* Mobile Overlay */}
          <div className="fixed inset-0 bg-black/50 z-50 sm:hidden" onClick={() => setIsOpen(false)} />
          ...
          <Card className="fixed bottom-16 right-0 left-0 h-[calc(100vh-6rem)] sm:bottom-6 sm:right-6 sm:left-auto sm:w-96 sm:h-[600px] sm:max-h-[calc(100vh-3rem)] shadow-2xl border-2 border-primary/20 z-[60] bg-background/95 backdrop-blur-sm sm:rounded-xl flex flex-col">
            <CardHeader className="flex flex-row items-center justify-between p-3 sm:p-4 border-b border-border bg-gradient-to-r from-primary via-primary/90 to-primary/80 text-primary-foreground sm:rounded-t-xl">
              <CardTitle className="text-base sm:text-lg font-semibold flex items-center gap-2">
                ...
                <span className="hidden sm:inline">Tech Sahayak (টেক সহায়ক)</span>
            ...
            <CardContent className="flex-1 p-0 flex flex-col overflow-hidden">
              <ScrollArea ref={scrollAreaRef} className="flex-1 px-3 sm:px-4 py-2 h-full">
                {chatHistory.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-center px-2">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground mb-4 sm:mb-6 max-w-xs">
                      আমি Tech Sahayak। আপনার পণ্য খোঁজা, পরামর্শ এবং অর্ডার ট্র্যাকিংয়ে সাহায্য করতে এখানে আছি।
                    </p>
                    
                    {/* Quick Actions */}
                    <div className="w-full mb-4 sm:mb-6">
                      <h4 className="text-xs sm:text-sm font-medium mb-2 sm:mb-3 text-left">Quick Actions</h4>
                        ...
                        {quickActions.map((action, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-left leading-tight text-xs">{action.label}</span>
                            </Button>
                          );
                        })}
                      </div>
                    ...
                    <div className="w-full">
                      <h4 className="text-xs sm:text-sm font-medium mb-2 sm:mb-3 text-left">Try asking:</h4>
                          ...
                          {chat.role === "model" && chat.reactions && (
                            <div className="flex items-center gap-1 ml-1 sm:ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleReaction(chat.id, 'helpful')}
                                className={cn(
                                  "h-6 sm:h-7 px-1.5 sm:px-2 text-xs",
                                  chat.reactions.helpful && "bg-green-100 text-green-700 hover:bg-green-200"
                                )}
                              >
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleReaction(chat.id, 'notHelpful')}
                                className={cn(
                                  "h-6 sm:h-7 px-1.5 sm:px-2 text-xs",
                                  chat.reactions.notHelpful && "bg-red-100 text-red-700 hover:bg-red-200"
                                )}
                              >
              ...
              {isTyping && (
                <div className="px-3 sm:px-4 pb-2">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="hidden sm:inline">Tech Sahayak is typing...</span>
