...
import {
  BangladeshiPhoneValidation,
  BANGLADESHI_OPERATORS,
} from "@/lib/types/user";
 ...
 */
export function validateBangladeshiPhone(
  phone: string,
): BangladeshiPhoneValidation {
  if (!phone) {
    return {
...
function isValidBangladeshiFormat(phone: string): BangladeshiPhoneValidation {
  ...
  if (phone.length < 10) {
    return {
      isValid: false,
      message:
        "Phone number is too short. Bangladeshi numbers should have at least 10 digits.",
    };
  }
  ...
  if (phone.length > 13) {
    return {
      isValid: false,
      message:
        "Phone number is too long. Please enter a valid Bangladeshi number.",
    };
  }
  ...
  if (/^01[0-2]/.test(phone)) {
    return {
      isValid: false,
      message:
        "Invalid operator code. Bangladeshi mobile numbers start with 013, 014, 015, 016, 017, 018, or 019.",
    };
  }
  ...
  if (/^880[0-2]/.test(phone)) {
    return {
      isValid: false,
      message:
        "Invalid operator code. Please enter a valid Bangladeshi mobile number.",
    };
  }
 ...
 */
export function getBangladeshiOperator(
  phone: string,
  ...
  for (const [_operatorKey, operatorInfo] of Object.entries(
    BANGLADESHI_OPERATORS,
  )) {
 ...
 */
export function formatBangladeshiPhone(
  phone: string,
