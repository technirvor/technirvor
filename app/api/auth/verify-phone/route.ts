...
import { PhoneVerificationData, PhoneVerificationResponse } from '@/lib/types/user';
  ...
  try {
    ...
    if (verificationError || !verification) {
      ...
      await supabase
        .from('phone_verifications')
        .update({ 
          attempts: supabase.rpc('increment_attempts', { phone_number: normalizedPhone })
        })
        .eq('phone', normalizedPhone)
        .eq('is_verified', false);
    ...
    if (user) {
      ...
      if (referral) {
        ...
        await supabase
          .from('user_rewards')
          .update({
            total_points: supabase.rpc('increment_points', { points: referralBonus }),
            available_points: supabase.rpc('increment_points', { points: referralBonus }),
            total_earned: supabase.rpc('increment_points', { points: referralBonus })
          })
        ...
        await supabase
          .from('user_rewards')
          .update({
            total_points: supabase.rpc('increment_points', { points: referredBonus }),
            available_points: supabase.rpc('increment_points', { points: referredBonus }),
            total_earned: supabase.rpc('increment_points', { points: referredBonus })
          })
          .eq('user_id', user.id);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (countError) {
      ...
      return NextResponse.json(
        { success: false, message: 'Too many verification codes sent. Please try again later.' },
        { status: 429 }
      );
    }
    ...
    console.log(`New verification code for ${normalizedPhone}: ${verificationCode}`);
