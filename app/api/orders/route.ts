...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!customer_name || !customer_phone || !district || !address || !items || !total_amount) {
    ...
    if (!Array.isArray(productIds) || productIds.length === 0) {
      return NextResponse.json({ error: 'No product IDs found in order items' }, { status: 400 });
    }
    ...
    if (!products || products.length === 0) {
      return NextResponse.json({ error: 'No products found for provided IDs' }, { status: 400 });
    }

    const missingIds = productIds.filter(
      (id: string) => !products.some((product: { id: string }) => product.id === id),
    );
    ...
    for (const item of items as OrderItem[]) {
      const product = products.find(
        (p: { id: string; stock: number; name: string }) => p.id === item.product_id,
      );
    ...
    for (const item of items) {
      ...
      const { data: stockUpdateResult, error: stockError } = await supabase.rpc('decrement_stock', {
        product_id: item.product_id,
        quantity: item.quantity,
      });
    ...
    await notificationService.notifyNewOrder(order.id, customer_name, total_amount);
