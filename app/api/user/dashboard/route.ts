...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalEarnedFromReferrals = referrals?.reduce((sum, r) => sum + (r.reward_points || 0), 0) || 0;

    // Calculate order stats
    const totalOrders = recentOrders?.length || 0;
    const totalSpent = recentOrders?.reduce((sum, order) => sum + (order.total_amount || 0), 0) || 0;
    ...
    const dashboardData: UserDashboardData = {
      ...
      rewards: {
        ...
        tier_progress: currentTier ? Math.min(100, ((rewards?.total_points || 0) / (currentTier.max_points || 1)) * 100) : 0,
